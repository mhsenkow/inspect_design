...
const ThemeSwitcher: React.FC<ThemeSwitcherProps> = ({ className = '' }) => {
  ...
  const applyTheme = (theme: string, spacing: string, radius: string, shadow: string) => {
    const body = document.body;
    
    // Remove all theme classes
    body.classList.remove(
      'theme-blue', 'theme-green', 'theme-purple', 'theme-orange', 'theme-red', 'theme-teal', 'theme-dark',
      'spacing-compact', 'spacing-relaxed',
  ...
  return (
    <div className={`theme-switcher ${className}`}>
      ...
      >
        {isOpen ? '▼ Hide Theme Switcher' : '▲ Show Theme Switcher'}
      </button>
        ...
        <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-lg shadow-lg border border-gray-200 p-4 z-50">
          ...
          <div className="mt-4 p-3 bg-gray-50 rounded-md">
            <p className="text-sm text-gray-600">
              <strong>Current Theme:</strong> {themes.find(t => t.class === currentTheme)?.name || 'Default'}
              {currentSpacing && ` + ${spacingOptions.find(s => s.class === currentSpacing)?.name}`}
              {currentRadius && ` + ${radiusOptions.find(r => r.class === currentRadius)?.name}`}
              {currentShadow && ` + ${shadowOptions.find(sh => sh.class === currentShadow)?.name}`}
            </p>
          </div>
        </div>
      )}
